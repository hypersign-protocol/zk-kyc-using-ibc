...
pub fn do_ibc_packet_receive(
    ...
    let msg: IbcQueryMsg = from_binary(&msg.packet.data)?;

    match msg {
        IbcQueryMsg::Verify { proof, public_signal, .. } => execute_query(proof, public_signal)
    }
}

fn execute_query(proof: String, public_signal: u64) -> Result<IbcReceiveResponse, ContractError> {
    let is_valid_address = process_proof(proof, public_signal);
    Ok(IbcReceiveResponse::new()
        .add_attribute("method", "execute_query")
        .set_ack(make_ack_success(is_valid_address)))
}
